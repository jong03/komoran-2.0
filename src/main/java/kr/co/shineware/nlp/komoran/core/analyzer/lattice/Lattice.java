...
package kr.co.shineware.nlp.komoran.core.analyzer.lattice;
...
public class Lattice {
	...
	public void init() {
		...
		LatticeNode initNode = getLatticeNode(
				SYMBOL.START, // morph 
				SYMBOL.START,        // posId
				0); // score   
			...
			if(this.nbest > 1){
				...
				for (LatticeNode maxPrevNode : nBestPrevNodeList) {
					...
					if(maxPrevNode == null){
						...
						LatticeNode curNode = getLatticeNode(
								morph, // morph 
								lastPosId,        // posId
								beginIdx,          // prevId
								maxPrevNode.hashCode(),  // hashCode
								maxPrevNode.getScore()+this.transition.get(maxPrevNode.getPosId(), firstPosId)+score); // score   
	...
	public List<List<List<Pair<String, String>>>> getNbest(String in) {
		...
		lastNodes = getLastNode(lastNodes,in);
	...
	private Map<Integer,LatticeNode> getLastNode(Map<Integer,LatticeNode> lastNodes, String in){
		...
		if(lastNodes == null){
			...
			for (Integer prevHashcode : prevHashSet) {
				...
				LatticeNode notAnalyzedNode = getLatticeNode(
						in.substring(this.getPrevStartIdx(),totalIdx), // morph 
						SYMBOL.NA,        // posId
						this.getPrevStartIdx(),          // prevId
	...
	private List<List<List<Pair<String, String>>>> backTrackingNbest(){
		...
		for(int j=0;j<nbest;j++){
			...
			List<List<Pair<String,String>>> sentenceResult = this.backTracking(trackNodes);
	...
	private List<List<Pair<String, String>>> backTracking() {
		 return this.backTracking(null);
	}

	private List<List<Pair<String, String>>> backTracking(Set<LatticeNode> trackNodes) {
	...
	private void updateEndTransition(Map<Integer,LatticeNode> lastNodeMap, String in) {
		...
		for (Entry<Integer, LatticeNode> lastNodeEntry : lastNodeSet) {
			...
			LatticeNode endNode = getLatticeNode(
					SYMBOL.END, // morph 
					SYMBOL.END,        // posId
					totalIdx,          // prevId
					lastNode.hashCode(),  // hashCode
					lastNodeScore+transitionScore); // score  
		...
		if(!isConnectedEndNode){
			...
			LatticeNode naNode = 	getLatticeNode(
					in.substring(this.prevStartIdx, this.totalIdx), // morph 
					SYMBOL.NA,        // posId
					this.prevStartIdx,          // prevId
					lattice.get(this.prevStartIdx).keySet().iterator().next(),  // hashCode
					SCORE.NA); // score   
	...
	public void bridgingSpace(String in, int i) {
		...
		if(lastNodeMap == null){
			...
			for (Integer prevHashcode : prevHashSet) {
				...
				LatticeNode notAnalyzedNode = getLatticeNode(
						in.substring(this.getPrevStartIdx(),i), // morph 
						SYMBOL.NA,        // posId
						this.getPrevStartIdx(),          // prevId
						prevHashcode,  // hashCode
						SCORE.NA); // score   
				...
				LatticeNode bridgeNode = getLatticeNode(
						SYMBOL.SPACE, // morph 
						SYMBOL.START,        // posId
						i,          // prevId
						notAnalyzedNode.hashCode(),  // hashCode
						0.0); // score    
			...
			if(maxPrevNode != null){
				...
				LatticeNode naNode = getLatticeNode(
						in.substring(this.prevStartIdx, i), // morph 
						SYMBOL.NA,        // posId
						this.prevStartIdx,          // prevId
						lattice.get(this.prevStartIdx).keySet().iterator().next(),  // hashCode
						SCORE.NA); // score      
	...
	public LatticeNode getLatticeNode(String space, String start, double score){
		return  getLatticeNode(space,  start, null,  score);
	}
	
	public LatticeNode getLatticeNode(String space, String start, Integer idx, double score){
		return  getLatticeNode(space,  start, idx, null,  score);
	}

	public LatticeNode getLatticeNode(String space, String start, Integer idx, Integer hashCode, double score){
		
		return getLatticeNode(space, this.table.getId(start), idx, hashCode, score);

	}
	
	public LatticeNode getLatticeNode(String space, int start, Integer idx, Integer hashCode, double score){
		LatticeNode bridgeNode = new LatticeNode(); //SPACE
		bridgeNode.setMorph(space);
		bridgeNode.setPosId(start);
		if (idx != null){
			bridgeNode.setPrevIdx(idx);
		}
		if(hashCode != null){
			bridgeNode.setPrevHashcode(hashCode);
		}
		bridgeNode.setScore(score);
